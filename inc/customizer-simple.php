 ...
 * Return this array in a function and add_filter to 'indira_customizer_wrapper_settings' hook
 * Create customizer settings and controls based the arguments
 * with filter 'indira_customizer_wrapper_settings'
 */
function indira_customizer_wrapper_settings( $wp_customize ) {

	do_action('indira_before_customize_wrapper', $wp_customize );

	$settings = array();
	$customize_admin_js_var = array();

	$settings = apply_filters( 'indira_customizer_wrapper_settings', $settings );
	$i = 1;
	...
	do_action('indira_after_customize_wrapper', $wp_customize );
}
add_action( 'customize_register', 'indira_customizer_wrapper_settings', 100, 1 );


/** 
 * Apply the CSS set by the customizer framework based on arguments 
 */
function indira_apply_customizer_css(){

	$settings = array();
	$settings = apply_filters( 'indira_apply_customizer_css', $settings );
	...
	if ( is_customize_preview() ) {
		wp_enqueue_style( 'indira-preview-style', get_template_directory_uri() . '/css/customize-preview.css', array( 'indira-style' ), '20160816' );
		wp_add_inline_style( 'indira-preview-style', $css );
		//wp_add_inline_style( 'indira-style', esc_attr($css) );
	} else {
		wp_add_inline_style( 'indira-style', $css );
	}
	wp_localize_script('indira-customize-preview', '_customizerCSS', $customize_js_var );

}
add_action('wp_enqueue_scripts', 'indira_apply_customizer_css', 11 );


/**
 * Binds JS handlers to make Theme Customizer preview reload changes asynchronously.
 */
function indira_customize_preview_js() {
	wp_enqueue_script( 'indira-customize-preview', get_template_directory_uri() . '/js/customize-preview.js', array( 'customize-preview' ), '20130508', true );
}
add_action( 'customize_preview_init', 'indira_customize_preview_js' );

/**
 * Binds JS handlers for the helper in the customizer admin.
 */
function indira_customize_admin_js() {
	wp_enqueue_script( 'indira-customizer-admin', get_template_directory_uri() . '/js/customize-control.js', array( ), '20141114', true );
}
add_action( 'customize_controls_enqueue_scripts', 'indira_customize_admin_js' );