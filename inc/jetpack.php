 ...
 * @since Indira 1.0
 */
function indira_remove_jp_share( $domain ) {
	remove_filter( 'the_content', 'sharing_display',19 );
	remove_filter( 'the_excerpt', 'sharing_display',19 );
}
add_action( 'loop_start', 'indira_remove_jp_share' );
 ...
 * @since Indira 1.0
 */
function indira_remove_jp_related( $domain ) {
...
add_action( 'loop_start', 'indira_remove_jp_related' );
 ...
 */
function indira_jetpack_sharing_display_text( $text, $service, $id, $args ) {
	if ( class_exists( 'Jetpack_Options' ) && Jetpack_Options::get_option_and_ensure_autoload( 'sharedaddy_disable_resources', '0' ) ) {
		$text = indira_svg_icon( 'social-' . $service->get_id() ) . '</span><span class="screen-reader-text">' . $text ;
	}

	return $text;
}
add_filter( 'jetpack_sharing_display_text', 'indira_jetpack_sharing_display_text', 4, 10 );
 ...
 * @param array $args Array of sharing service options.
 */
function indira_jetpack_sharing_display_classes( $klasses, $service, $id, $args ) {
	if ( class_exists( 'Jetpack_Options' ) && Jetpack_Options::get_option_and_ensure_autoload( 'sharedaddy_disable_resources', '0' ) ) {
...
add_filter( 'jetpack_sharing_display_classes', 'indira_jetpack_sharing_display_classes', 4, 10 );
 ...
 *
 */
function indira_jetpack_inline_css() {
...
add_action( 'wp_enqueue_scripts', 'indira_jetpack_inline_css', 10 );
