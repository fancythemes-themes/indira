...
( function( $ ) {
	...
	var style = $( '#indira-color-scheme-css' ),
		api = wp.customize;

	if ( ! style.length ) {
		style = $( 'head' ).append( '<style type="text/css" id="indira-color-scheme-css" />' )
		                    .find( '#indira-color-scheme-css' );
	}
	...
	function writeCSS(){
		...
		$('#indira-preview-style-inline-css').text(cssOutput);
	}
